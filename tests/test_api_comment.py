...
class TestCommentAPI:
    ...
    def test_comments_not_authenticated(self, client, post_1):
        response = client.get(f'/api/v1/posts/{post_1.id}/comments/')
    ...
    @pytest.mark.django_db(transaction=True)
    def test_comment_single_not_authenticated(self, client, post_1, comment_1_post):
        response = client.get(f'/api/v1/posts/{post_1.id}/comments/{comment_1_post.id}/')
    ...
    @pytest.mark.django_db(transaction=True)
    def test_comments_not_found(self, user_client_2, post_1):
        response = user_client_2.get(f'/api/v1/posts/{post_1.id}/comments/')
    ...
    @pytest.mark.django_db(transaction=True)
    def test_comments_get(self, user_client_2, post_1, comment_1_post, comment_2_post, comment_1_another_post):
        response = user_client_2.get(f'/api/v1/posts/{post_1.id}/comments/')
        ...
        assert len(test_data) == Comment.objects.filter(post=post_1).count(), (
            'Проверьте, что при GET запросе на `/api/v1/posts/{post.id}/comments/` '
            'возвращается весь список комментов статьи'
        )

        comment = Comment.objects.filter(post=post_1).first()
    ...
    @pytest.mark.django_db(transaction=True)
    def test_comments_create(self, user_client_2, post_1, user, another_user_2):
        comments_count = Comment.objects.count()

        data = {}
        response = user_client_2.post(f'/api/v1/posts/{post_1.id}/comments/', data=data)
        assert response.status_code == 400, (
            'Проверьте, что при POST запросе на `/api/v1/posts/{post.id}/comments/` '
            'с не правильными данными возвращается статус 400'
        )

        data = {'author': another_user_2.id, 'text': 'Новый коммент 1233', 'post': post_1.id}
        response = user_client_2.post(f'/api/v1/posts/{post_1.id}/comments/', data=data)
    ...
    @pytest.mark.django_db(transaction=True)
    def test_comment_get_current(self, user_client_2, post_1, comment_1_post, user):
        response = user_client_2.get(f'/api/v1/posts/{post_1.id}/comments/{comment_1_post.id}/')
        ...
        assert test_data.get('post') == post_1.id, (
            'Проверьте, что при GET запросе `/api/v1/posts/{post.id}/comments/{comment.id}/` '
            'возвращаете данные сериализатора, не найдено или не правильное значение `post`'
        )

    @pytest.mark.django_db(transaction=True)
    def test_comment_patch_current(self, user_client_2, post_1, comment_1_post, comment_2_post):
        response = user_client_2.patch(f'/api/v1/posts/{post_1.id}/comments/{comment_1_post.id}/',
        ...
        response = user_client_2.patch(f'/api/v1/posts/{post_1.id}/comments/{comment_2_post.id}/',
    ...
    @pytest.mark.django_db(transaction=True)
    def test_comment_delete_current(self, user_client_2, post_1, comment_1_post, comment_2_post):
        response = user_client_2.delete(f'/api/v1/posts/{post_1.id}/comments/{comment_1_post.id}/')

        assert response.status_code == 204, (
            'Проверьте, что при DELETE запросе `/api/v1/posts/{post.id}/comments/{comment.id}/` возвращаете статус 204'
        )

        test_comment = Comment.objects.filter(id=post_1.id).first()

        assert not test_comment, (
            'Проверьте, что при DELETE запросе `/api/v1/posts/{post.id}/comments/{comment.id}/` вы удалили комментарий'
        )

        response = user_client_2.delete(f'/api/v1/posts/{post_1.id}/comments/{comment_2_post.id}/')
