...
class TestFollowAPI:
    ...
    @pytest.mark.django_db(transaction=True)
    def test_follow_get(self, user_client_2, user, follow_1, follow_2, follow_3):
        response = user_client_2.get('/api/v1/follow/')
    ...
    @pytest.mark.django_db(transaction=True)
    def test_follow_create(self, user_client_2, follow_2, follow_3, user, user_2, another_user_2):
        follow_count = Follow.objects.count()

        data = {}
        response = user_client_2.post('/api/v1/follow/', data=data)
        assert response.status_code == 400, (
            'Проверьте, что при POST запросе на `/api/v1/follow/` с неправильными данными возвращается статус 400'
        )

        data = {'author': another_user_2.username}
        response = user_client_2.post('/api/v1/follow/', data=data)
    ...
    @pytest.mark.django_db(transaction=True)
    def test_follow_search_filter(self, user_client_2, follow_1, follow_2,
                                  follow_3, follow_4, follow_5,
                                  user, user_2, another_user_2):
        ...
        response = user_client_2.get(f'/api/v1/follow/?search={user_2.username}')
        ...
        response = user_client_2.get(f'/api/v1/follow/?search={another_user_2.username}')
        assert len(response.json()) == follow_user.filter(author=another_user_2).count(), (
            'Проверьте, что при GET запросе с параметром `search` на `/api/v1/follow/` '
            'возвращается результат поиска по подписке'
        )
