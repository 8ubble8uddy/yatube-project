...
class TestPostAPI:
    ...
    @pytest.mark.django_db(transaction=True)
    def test_post_list_not_auth(self, client, post_1):
        response = client.get('/api/v1/posts/')
    ...
    @pytest.mark.django_db(transaction=True)
    def test_post_single_not_auth(self, client, post_1):
        response = client.get(f'/api/v1/posts/{post_1.id}/')
    ...
    @pytest.mark.django_db(transaction=True)
    def test_posts_get(self, user_client_2, post_1, another_post):
        response = user_client_2.get('/api/v1/posts/')
        assert response.status_code == 200, (
    ...
    @pytest.mark.django_db(transaction=True)
    def test_post_create(self, user_client_2, user, another_user_2, group_1):
        post_count = Post.objects.count()

        data = {}
        response = user_client_2.post('/api/v1/posts/', data=data)
    ...
    @pytest.mark.django_db(transaction=True)
    def test_post_get_current(self, user_client_2, post_1, user):
        response = user_client_2.get(f'/api/v1/posts/{post_1.id}/')
        ...
        test_data = response.json()
        assert test_data.get('text') == post_1.text, (
    ...
    @pytest.mark.django_db(transaction=True)
    def test_post_patch_current(self, user_client_2, post_1, another_post):
        response = user_client_2.patch(f'/api/v1/posts/{post_1.id}/',
        ...
        test_post = Post.objects.filter(id=post_1.id).first()
        ...
        response = user_client_2.patch(f'/api/v1/posts/{another_post.id}/',
                                     data={'text': 'Поменяли текст статьи'})
    ...
    @pytest.mark.django_db(transaction=True)
    def test_post_delete_current(self, user_client_2, post_1, another_post):
        response = user_client_2.delete(f'/api/v1/posts/{post_1.id}/')
        ...
        response = user_client_2.delete(f'/api/v1/posts/{another_post.id}/')
