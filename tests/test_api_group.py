...
class TestGroupAPI:
    ...
    def test_group_not_found(self, client, post_1, group_1):
    ...
    @pytest.mark.django_db(transaction=True)
    def test_group_list_not_auth(self, client, post_1, group_1):
        response = client.get('/api/v1/groups/')
    ...
    @pytest.mark.django_db(transaction=True)
    def test_group_get(self, user_client_2, post_1, another_post, group_1, group_2):
        response = user_client_2.get('/api/v1/groups/')
        assert response.status_code == 200, (
    ...
    @pytest.mark.django_db(transaction=True)
    def test_group_cannot_create(self, user_client_2, group_1, group_2):
        group_count = Group.objects.count()

        data = {}
        response = user_client_2.post('/api/v1/groups/', data=data)
    ...
    @pytest.mark.django_db(transaction=True)
    def test_group_get(self, user_client_2, post_1, post_2, another_post, group_1, group_2):
        response = user_client_2.get('/api/v1/groups/')
        assert response.status_code == 200, (
            'Страница `/api/v1/groups/` не найдена, проверьте этот адрес в *urls.py*'
        )
        ...
        response = user_client_2.get(f'/api/v1/groups/{group_2.id}/')
        ...
        response = user_client_2.get(f'/api/v1/groups/{group_1.id}/')
        assert isinstance(response.json(), dict), (
            'При запросе `/api/v1/groups/{id}/` должен возвращаться словарь'
        )
